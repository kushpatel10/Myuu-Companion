...
function createBotInfoEmbed(client) {
 ...
 function createServerInfoEmbed(guild, owner, interaction, settings) {
      
      let markovMode = 'Random'; 
      let markovChannels = 'None'; 
      
      if (settings) {
        if (Array.isArray(settings.channels) && settings.channels.includes(interaction.channelId)) {
          markovMode = settings.mode || 'Random';  
        } else {
          markovChannels = 'No Markov enabled channels';
        }
      }
      
      const createdAt = guild.createdAt instanceof Date ? guild.createdAt.toDateString() : 'Unknown';
      
      let description = `🏰 **Server Name**: ${guild.name}
        👑 **Server Owner**: ${owner.user.tag}
        👥 **Members**: ${guild.memberCount}
        🌍 **Region**: ${guild.preferredLocale}
        🎮 **Created At**: ${createdAt}
        🧠 **Markov Mode**: ${markovMode}`;
      
      if (markovChannels !== 'None') {
        description += ` | 📡 **Channels**: ${settings.channels.join(', ')}`;
      }
